...
const UpdateProduct = async (req, res) => {
  try {
    ...
    } //input tag name , object.schema name, input tag name
    if (product_code) {
      newProduct.product_code = product_code;
    }
    if (tax_code) {
      newProduct.tax_code = tax_code;
    }
    if (product_description) {
      newProduct.product_description = product_description;
    }
    if (rack_no) {
      newProduct.rack_no = rack_no;
    }
    if (HSN) {
      newProduct.HSN = HSN;
    }
    if (product_barcode) {
      newProduct.product_barcode = product_barcode;
    }
    if (category) {
      newProduct.category = category;
    }
    if (unit_of_masure) {
      newProduct.unit_of_masure = unit_of_masure;
    }
    if (manufactures) {
      newProduct.manufactures = manufactures;
    }
    if (weight_dimension) {
      newProduct.weight_dimension = weight_dimension;
    }
    if (variants) {
      newProduct.variants = variants;
    }
    if (reorder_point) {
      newProduct.reorder_point = reorder_point;
    }
    if (active_status) {
      newProduct.active_status = active_status;
    }
    let updatedGrocery = await Product.findByIdAndUpdate(
      id,
      { $set: newProduct },
      { new: true }
    );
